...
class Base_Models {
    ...
    public function __construct($module_name = null, $table_name = null) {
        ...
        $this->_database_config = get_config_by_key('database')->database->master->toArray();
    ...
    public function getMasterConnection() {
        ...
        if (!isset(self::$_master_connections[$module_name])) {
            self::$_master_connections[$module_name] = Zend_Db::factory($this->_database_config['adapter'], $this->_database_config['params']);
        }
    ...
    public function insert(array $data, $table_name ="") {
        ...
        //连接主库, 执行插入操作, 返回插入的行数
        $affected_rows = $this->getMasterConnection()->insert($table_name, $data);
     ...
     * @return bool|int
     */
    public function delete($where = null,$table_name = "") {
        //验证$where
        if ($where === null) {
            $condition = $this->_getCondition();
        ...
        //连接主库, 执行删除操作, 返回影响的行数
        $affected_rows = $this->getMasterConnection()->delete($table_name, $where);
